...
# 文件夹路径
image_folder = "/root/autodl-tmp/train_png/"
flow_folder = "/root/autodl-tmp/result/inference/run.epoch-0-flow-field/"
save_folder = "/root/autodl-tmp/result/feature_maps/"
...
def collect_images(image_folderr):
    ...
    for i in range(len(sorted_png)-2):
        ...
        match1 = re.match(r'lax_P(\d+)_slice_(\d+)_time_(\d+)\.png', file1)
        match2 = re.match(r'lax_P(\d+)_slice_(\d+)_time_(\d+)\.png', file2)
        match3 = re.match(r'lax_P(\d+)_slice_(\d+)_time_(\d+)\.png', file3)

        # 确保两个文件名都匹配模式
        if match1 and match2 and match3:
            P_1 = int(match1.group(1))
            P_2 = int(match2.group(1))
            P_3 = int(match3.group(1))
            slice_num1 = int(match1.group(2))
            slice_num2 = int(match2.group(2))
            slice_num3 = int(match3.group(2))
            # 检查P值和slice是否相同
            if slice_num1 == slice_num2 and P_1 == P_2 and P_1 == P_3 and slice_num1 == slice_num3 and P_1 >=106:
                # 获取 time 编号
                fnum1 = int(match1.group(3))
                fnum2 = int(match2.group(3))
                fnum3 = int(match3.group(3))
                
                # 打印slice和time编号调试用
                print(f" P: {P_1}, Slice: {slice_num1}, Time1: {fnum1}, Time2: {fnum2},Time3: {fnum3}")
...
for i in tqdm(range(len(image_list)-2)):
    ...
    # 生成保存路径
    feature_map_t_minus_1_path = os.path.join(save_folder, f"fm_t_minus_1_{file_name_0}.png")
    feature_map_t_plus_1_path = os.path.join(save_folder, f"fm_t_plus_1_{file_name_2}.png")
