...
def compute_metrics(original_tensor, dct_tensor):
    ...
    for orig, recon in zip(original_images, reconstructed_images):
        ...
        # 计算 SSIM
        ssim_value = compare_ssim(orig, recon,channel_axis=2, data_range=1.0)
...
# 训练过程
def train_model(model, dataloader, criterion, optimizer, num_epochs, test_ratio=0.15, batch_size=8):
    ...
    #model.load_state_dict(torch.load('/root/autodl-tmp/.autodl/dataset1-70/transformer/transformer_last.pth'))
    model.train()
    for epoch in range(num_epochs):
        running_loss = 0.0
        ssim = 0
        psnr = 0
        for k, (train_img, train_img_tar, flow_0, flow_1) in tqdm(enumerate(train_loader)):
            optimizer.zero_grad()
        ...
        with torch.no_grad():
            for k, (val_img, val_img_tar, flow_0, flow_1) in tqdm(enumerate(test_loader)):
                # 假设输入的形状是 [seq_len, batch_size, input_dim]
                # 这里我们需要调整输入的维度
        ...
        with open("/root/autodl-tmp/.autodl/dataset1-70/transformer/validation_metrics.txt", "a") as file:
          ...
          # 保存模型权重
        weight_path = f"/root/autodl-tmp/.autodl/dataset1-70/transformer/model_weights_epoch_{epoch+1}.pth"
