...
def temp_seed(rng, seed):
    state = rng.get_state()
    # 确保 seed 是一个整数并在 CPU 上
    if isinstance(seed, torch.Tensor):
        seed = seed.cpu().item()  # 将张量转换为 CPU 并获取其值
    rng.seed(seed)
    try:
        yield
    finally:
...
class RandomMaskFunc(MaskFunc):
    ...
    def __call__(self, shape, seed=None):
        ...
        with temp_seed(self.rng, seed):
            ...
            # 计算高频采样概率
            prob_high_freq = (num_cols / acceleration - num_low_freqs) / (num_cols - num_low_freqs)
            # print('prob_high_freq', prob_high_freq)
            ...
            mask[..., 0 : num_low_freqs] = 1.0  # 低频位置设为1

            # 随机选择高频位置
            high_freq_mask_location = np.random.uniform(size=num_cols) < prob_high_freq
            high_freq_mask_location = np.repeat(high_freq_mask_location[:, np.newaxis], 3, axis=1)


            # 将高频位置设为1
            mask[..., high_freq_mask_location] = 1.0
        return mask
